...
## Escopo
- **Dentro**: `ocr.py::create_ocr_engine` aceitando 5 nomes — `rapidocr-openvino`, `rapidocr-onnxruntime`, `paddleocr`, `winocr`, `tesseract`; a lista única `ocr.py::ENGINE_CHOICES` e o default único `ocr.py::DEFAULT_ENGINE`, consumidos por **todos** os pontos de entrada (`gui.py`, `perception/run.py`, `main.py`, `watcher.py`, `study_ocr_engines.py`) e também pelo nível de biblioteca (`perception.perceive()`/`default_pipeline()`, `perception/stages/e2_extraction.py`); `study_ocr_engines.py` (raiz) como comparador.
